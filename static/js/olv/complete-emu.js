...
if (typeof wiiuBrowser === 'undefined') {
    window.wiiuBrowser = {
        ...
        openTvAreaSetting: function () {
            alert('Open TV area setting screen');
        },
        jumpToBrowser: function (url) {
            alert("Jump to URL: " + url);
        },
        jumpToEshop: function (query) {
            alert("Jump to eShop with query: " + query);
        },
        jumpToTvii: function () {
            alert("Jump to TVii");
        },
        jumpToApplication: function(titleId, flags, communityId, appData, postId) {
            alert("Jump to application: " + titleId );
        }
    };
}
...
if (typeof wiiuSystemSetting === 'undefined') {
    window.wiiuSystemSetting = {
        ...
        getRegion: function () {
            if (!wiiuSystemSetting.region) {
                wiiuSystemSetting.region = iso31661Alpha2ToAlpha3[navigator.language.split("-")[1]];
            }

            return {
                code: wiiuSystemSetting.region
            };
        },
        getCountry: function () {
            if (!wiiuSystemSetting.country) {
                wiiuSystemSetting.country = navigator.language.split("-")[1];
            }
            
            return {
                code: wiiuSystemSetting.country
            };
        },
        getLanguage: function () {
            if (!wiiuSystemSetting.language) {
                wiiuSystemSetting.language = navigator.language.split("-")[0];
            }

            return {
                code: wiiuSystemSetting.language
            };
        },
...
if (typeof wiiuBOSS === 'undefined') {
    window.wiiuBOSS = {
        ...
        registerBossTask: function (languageCode) {
            console.log('wiiuBOSS.registerBossTask code: ' + languageCode);
        ...
        registerDirectMessageTask: function (languageCode) {
            console.log('wiiuBOSS.registerDirectMessageTask code ' + languageCode);
            //return { "error" : { "code" : 1112222, "message" : "Test Error Message"} };
            return {};
        },
        registerDirectMessageTaskEx: function (lifeTime, interval) {
            console.log('wiiuBOSS.registerDirectMessageTaskEx interval: ' + interval + ' lifeTime: ' + lifeTime);
            //return { "error" : { "code" : 1112222, "message" : "Test Error Message"} };
            return {};
        },
...
if (typeof wiiuMainApplication === 'undefined') {
    window.wiiuMainApplication = {
        ...
        getExternalImageData:  function () { return this.getScreenShot(true); },
